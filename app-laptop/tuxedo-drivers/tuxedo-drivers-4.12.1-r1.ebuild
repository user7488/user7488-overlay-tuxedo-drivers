...
src_install() {
    ...
    # Install udev rules
    install -Dm644 "${S}/99-z-tuxedo-systemd-fix.rules" \
        "${D}/usr/lib/udev/rules.d/99-z-tuxedo-systemd-fix.rules"

    install -Dm644 "${S}/99-infinityflex-touchpanel-toggle.rules" \
        "${D}/usr/lib/udev/rules.d/99-infinityflex-touchpanel-toggle.rules"

    # Install hwdb file
    install -Dm644 "${S}/61-sensor-infinityflex.hwdb" \
        "${D}/usr/lib/udev/hwdb.d/61-sensor-infinityflex.hwdb"
}
